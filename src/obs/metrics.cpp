...
void metrics_observer::on_compensation_start(std::size_t /*exec_id*/, std::size_t /*node_id*/,
                                             std::string_view compensate_task_type, std::int32_t /*attempt*/) noexcept {
  std::lock_guard lock(mutex_);
  metrics_[std::string(compensate_task_type)].start_count++;
}

void metrics_observer::on_compensation_complete(std::size_t /*exec_id*/, std::size_t /*node_id*/,
                                                std::string_view compensate_task_type,
                                                std::chrono::milliseconds duration_ms) noexcept {
  std::lock_guard lock(mutex_);
  auto& m = metrics_[std::string(compensate_task_type)];
  m.success_count++;
  m.total_duration_ms += duration_ms;
  m.min_duration_ms = std::min(m.min_duration_ms, duration_ms);
  m.max_duration_ms = std::max(m.max_duration_ms, duration_ms);
...
void metrics_observer::on_compensation_fail(std::size_t /*exec_id*/, std::size_t /*node_id*/,
